...
- (추가구현) 모든 면을 맞추면 축하메시지와 함께 프로그램 종료 (완성)
  - 각 면의 1,1 요소는 움직이지 않게 설계되어 있으므로 현재 cube가 initCube와 동일한지 비교하면 된다.
  - isComplete() 중심으로 구현.
    - 처음 안 사실 : .equals()만으로는 이차원배열을 비교할 수 없다..
    - 문제 : 비교할 큐브를 만드는 과정이 너무 initCube와 비슷하다. 통합하고싶다-> initCube() 리팩토링과 함꼐 해결
      - Object.deepEquals()를 사용하거나 반복문으로 한줄씩 equals()로 비교해야 한다.
      - initCube()와 동일한 방식으로 초기화한 임시큐브와 비교.
      
**TODO**
- step1, step2, step3 리팩토링 : 가능한 요구조건에 맞게.
- README.md 깔끔하고 가독성 높아지게 수정
  - '구현한 코드 동작에 대해 정리한다' 
- 지원서에 쓸 내용 생각 및 정리해두기
